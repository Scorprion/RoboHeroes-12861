...
public class HermesEncoderTest extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        double right_avg, left_avg, right_difference, left_difference;

        while(opModeIsActive()) {
            right_avg = (initial_frontright + initial_backright) / 2;
            left_avg = (initial_frontleft + initial_backleft) / 2;
            ...
            right_difference = Math.abs(right_avg - (robot.FrontRight.getCurrentPosition() + robot.BackRight.getCurrentPosition()) / 2);
            left_difference = Math.abs(left_avg - ((robot.FrontLeft.getCurrentPosition() + robot.BackLeft.getCurrentPosition()) / 2));
